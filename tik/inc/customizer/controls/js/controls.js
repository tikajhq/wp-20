		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="tik-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="tik-input-container" data-id="' + data.id + '">' + html + '</div>'
				);
			},
			...
			init: function ( control ) {
				var picker = jQuery(
					'.tik-color-control[data-id="' + control.id + '"]'
					),
					clear;
				...
				setTimeout( function () {
					clear = jQuery( '.tik-input-container input[data-id="' + control.id + '"]' ).parent().siblings( '.wp-picker-clear' );
...
(
	function () {
		 ...
		 */
		wp.customize.tikDynamicControl = wp.customize.Control.extend( {
			...
			ready: function () {
				...
				control.deferred.embedded.done( function () {
					control.initTikControl( control );
				} );
			},
			...
			embed: function () {
				...
				wp.customize.section( sectionId, function ( section ) {
					if (
						'tik-expanded' === section.params.type ||
						section.expanded() ||
			 ...
			 * @returns {void}
			 */
			initTikControl: function ( control ) {
				if ( 'undefined' !== typeof tik.control[control.params.type] ) {
					tik.control[control.params.type].init( control );
					return;
				}
			...
			tikValidateCSSValue: function ( value ) {
				var validUnits = [
					    'rem',
					    'em',
					    'ex',
...
_.each( tik.control, function ( obj, type ) {
	wp.customize.controlConstructor[
		type
		] = wp.customize.tikDynamicControl.extend( {} );
} );

/**
 * Control: FontAwesome.
 */
/* global tikControlLoader, tikFontAwesome */
wp.customize.controlConstructor['tik-fontawesome'] = wp.customize.Control.extend(
	{
		ready: function () {
			'use strict';
			...
			// Init the control.
			if ( ! _.isUndefined( window.tikControlLoader ) && _.isFunction( tikControlLoader ) ) {
				tikControlLoader( control );
			} else {
				control.initTikControl();
			}
		},

		initTikControl: function () {
			var control       = this,
			    faSelect,
			    elSelector,
			    ...
			    faData        = [],
			    value         = control.setting._value,
			    data          = window['tikFontAwesome' + this.id],
			    faDataCounter = 0;
			...
			faSelect.on( 'change', function () {
				// Set the value.
				tik.setting.set( control.id, elSelector.val() );
 ...
 * Control: Typography.
 */
/* global tikControlLoader, tikAllFonts */
wp.customize.controlConstructor['tik-typography'] = wp.customize.Control.extend( {
	...
	initTikControl: function () {
		...
		if ( !_.isUndefined( control.params.default.color ) ) {
			picker = this.container.find( '.tik-color-control' );
			picker.wpColorPicker( {
				change: function () {
	...
	getFonts: function () {
		...
		if ( !_.isUndefined( window['tikFonts' + control.id] ) ) {
			return window['tikFonts' + control.id];
		}
		if ( !_.isUndefined( tikAllFonts ) ) {
			return tikAllFonts;
		}
		return {
			google: [],
 ...
 * Control: Slider.
 */
wp.customize.controlConstructor['tik-slider'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		var control      = this,
		    ...
		    slider       = control.container.find( '.tik-slider' ),
 ...
 * Control: Text.
 */
wp.customize.controlConstructor['tik-text'] = wp.customize.Control.extend( {
 ...
 * Control: Sortable.
 */
/* global tikControlLoader */
wp.customize.controlConstructor['tik-sortable'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		// Init the control.
		if (
			!_.isUndefined( window.tikControlLoader ) &&
			_.isFunction( tikControlLoader )
		) {
			tikControlLoader( control );
		} else {
			control.initTikControl();
	...
	initTikControl: function () {
		...
		var control = this;
		control.container.find( '.tik-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['tik-radio-image'] = wp.customize.tikDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['tik-toggle'] = wp.customize.tikDynamicControl.extend( {
 ...
 */
wp.customize.controlConstructor['tik-radio-buttonset'] = wp.customize.Control.extend( {} );

/**
 * Control: Dimensions.
 */
/* global dimensionstikL10n */
/* global tikControlLoader */
wp.customize.controlConstructor['tik-dimensions'] = wp.customize.tikDynamicControl.extend( {
			...
			setting.bind( function ( value ) {
				...
				_.each( ['top', 'right', 'bottom', 'left'], function ( direction ) {
					if ( !_.isUndefined( value[direction] ) ) {
						if ( false === control.tikValidateCSSValue( value[direction] ) ) {
							subs[direction] = dimensionstikL10n[direction];
						} else {
				...
				if ( !_.isEmpty( subs ) ) {
					message = dimensionstikL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
 ...
 */
/* global tikControlLoader */
wp.customize.controlConstructor['tik-background'] = wp.customize.Control.extend( {
	...
	initTikControl: function () {
		...
		var control = this,
		    value   = control.setting._value,
		    picker  = control.container.find( '.tik-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['tik-editor'] = wp.customize.tikDynamicControl.extend( {
...
jQuery( document ).ready( function() {
	jQuery( '.tik-customizer-title-wrapper.toggle-title' ).on( 'click', function() {
		jQuery( this ).siblings( '.tik-typography-wrapper' ).slideToggle( 'slow' );
