...
/**
 * Class Tik_Customize_Base_Option
 */
class Tik_Customize_Base_Option {
	...
	/**
	 * Tik_Customize_Base_Option constructor.
	 */
	public function __construct() {

		// Register customizer options.
		add_action( 'customize_register', array( $this, 'tik_customizer_options' ) );
		// Generate styles from customizer inputs.
		add_action( 'wp_enqueue_scripts', array( $this, 'tik_enqueue_style' ) );
	...
	protected function get_menu_options() {
		...
		$menu_options         = array();
		$menu_options['none'] = esc_html__( 'None', 'tik' );
	 ...
	 */
	public function tik_customizer_options( $wp_customize ) {
		...
		foreach ( $this->elements as $el_key => $el_data ) :
			...
			if ( ! $is_custom_control ) {
				...
				$control_type    = 'Tik_Customize_' . $control_type_uc . '_Control';
	 ...
	 */
	public function tik_enqueue_style() {
		...
		if ( tik_is_tik_pro_active() ) {
			wp_add_inline_style( 'tik-pro', $output );
		} else {
			wp_add_inline_style( 'tik-style', $output );
		}

		wp_add_inline_style( 'tik-woocommerce-style', $wc_output );
		wp_add_inline_style( 'tik-pro', $pro_output );
		wp_add_inline_style( 'tik-pro', $pro_wc_output );
	}
